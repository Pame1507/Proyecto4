...
# ---
# 
# * Estudiante: **Pamela Guido Santana**
# * Carné: **B53240**
# * Grupo: **2**
# 
# ---
...
# Tiempo de muestreo 
t_muestreo = np.linspace(0, 0.1,100)

# Rango de valores de W
W = [1,-1]

# Posibles formas de onda 
x_t = np.empty((4, len(t_muestreo)))

plt.figure()

# Matriz con los valores de cada función   
for i in W:
    x1 = i * np.cos(2*(np.pi)*fc*t_muestreo) +  i* np.sin(2*(np.pi)*fc*t_muestreo)
    x2 = -i * np.cos(2*(np.pi)*fc*t_muestreo) +  i* np.sin(2*(np.pi)*fc*t_muestreo) 
    x_t[i,:] = x1
    x_t[i+1,:] = x2
    plt.plot(t_muestreo,x1, lw=2)
    plt.plot(t_muestreo, x2, lw=2)       

# Promedio de las 4 funciones en cada instante para t
Promedio = [np.mean(x_t[:,i]) for i in range(len(t_muestreo))]
plt.plot(t_muestreo, Promedio, lw=6,color='k',label='Funciones')

# Resultado teórico
E = np.mean(senal_Tx)*t_muestreo  # Valor esperado de la señal 
plt.plot(t_muestreo, E, ':', lw=3,color='m',label='Valor teórico')

# Gráfica de las funciones
plt.title('Funciones del proceso aleatorio $X(t)$')
plt.xlabel('$t$')
plt.ylabel('$x_i(t)$')
plt.legend()
plt.show()
...
# Tiempo del símbolo es igual al período de la onda portadora
Tc = 1/fc
